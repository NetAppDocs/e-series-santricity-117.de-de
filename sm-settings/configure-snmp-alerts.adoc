...
Wenn Sie fertig sind, klicken Sie auf *Hinzufügen*, und klicken Sie dann auf *Schließen*.
...
** *Authentifizierungsfehler senden Trap* -- Wählen Sie diese Option (das Kontrollkästchen) aus, wenn Sie das Trap-Ziel benachrichtigen möchten, wenn eine SNMP-Anfrage aufgrund eines nicht erkannten Community-Namens oder Benutzernamens abgelehnt wird.
Nach dem Klicken auf *Hinzufügen* werden die Trap-Ziele und die zugehörigen Namen auf der Seite *SNMP* auf der Registerkarte *Alarme* angezeigt.
